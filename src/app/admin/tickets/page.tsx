...
import {
  LifeBuoy, CheckCircle2, MessageSquare, Loader2, Send, Clock, X, AlertTriangle
} from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-white tracking-wide flex items-center gap-2">
        <LifeBuoy className="text-neon-blue animate-pulse" /> Support Tickets Management
      </h1>
          ...
          <div className="lg:col-span-1 space-y-4">
            <div className="glass-card overflow-hidden">
              <div className="p-4 border-b border-white/10 bg-white/5 flex items-center justify-between">
                <h2 className="font-semibold text-white">All Tickets ({tickets.length})</h2>
              </div>
                  ...
                  tickets.map((ticket) => (
                    <button
                      ...
                      className={`w-full text-left p-4 hover:bg-white/5 transition-colors block ${
                        selectedTicket?.id === ticket.id ? "bg-white/10 border-l-2 border-neon-blue" : ""
                      }`}
                    >
                      ...
                      <p className="text-xs text-neon-blue font-semibold truncate mb-1">{ticket.subject}</p>
              ...
              >
                ...
                <div className="flex-1 p-6 space-y-4 overflow-y-auto custom-scrollbar bg-slate-950/20">
                  ...
                  <div className="flex justify-start">
                    <div className="glass-card max-w-[80%] p-4 border border-neon-blue/20 bg-neon-blue/5">
                      <p className="text-xs text-neon-blue font-bold mb-1">Customer Original Message</p>
                  ...
                  {repliesLoading ? (
                    ...
                    replies.map((reply) => {
                      const isSelf = reply.userId !== selectedTicket.userId; // Admin is self
                      return (
                        <div key={reply.id} className={`flex ${isSelf ? "justify-end" : "justify-start"}`}>
                          ...
                          >
                            <p className={`text-xs font-bold mb-1 ${isSelf ? "text-purple-400" : "text-neon-green"}`}>
                ...
                {selectedTicket.status === "open" ? (
                  <form onSubmit={handleSendReply} className="p-4 border-t border-white/10 bg-white/5 flex gap-2">
                    ...
                    <button
                      ...
                      className="px-5 py-3 rounded-xl bg-neon-blue/20 text-neon-blue border border-neon-blue/30 hover:bg-neon-blue/30 transition-colors font-bold disabled:opacity-50 flex items-center justify-center shrink-0"
                    >
