...
async function receiveBill(formData: FormData) { 
  ...
  if (!customer || customer.adminId !== employee.adminId) return;
...
export default async function EmployeeCollectionPage() {
  ...
  const customers = await db.query.users.findMany({ 
    where: and(eq(users.role, "customer"), eq(users.adminId, employee.adminId)), 
    with: { package: true } 
  });

  const adminUsers = await db.select({ id: users.id }).from(users).where(eq(users.adminId, employee.adminId));
  const adminUserIds = adminUsers.map(u => u.id);

  const recent = await db.query.payments.findMany({ 
    where: inArray(payments.userId, adminUserIds.length > 0 ? adminUserIds : [-1]),
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-8">
      <form action={receiveBill} className="glass-card p-6 space-y-5">
        <h1 className="text-2xl font-bold text-white flex gap-2">
          <Banknote className="text-neon-green"/> Receive Customer Bill
        </h1>
        <select name="userId" required className="w-full glass-input px-4 py-3 bg-slate-800">
        ...
        <input name="amount" type="number" required placeholder="Amount received" className="w-full glass-input px-4 py-3 bg-slate-800"/>
        <button className="w-full py-3 rounded-xl bg-neon-green/20 text-neon-green border border-neon-green/40 font-semibold">
          Receive & Activate Line
        </button>
        <p className="text-xs text-gray-500">This does not expose total company income; it only records staff collection.</p>
      </form>

      <div className="glass-card overflow-hidden">
        <div className="p-5 border-b border-white/10 bg-white/5">
          <h2 className="text-white font-semibold">Recent Staff Collections</h2>
        </div>
        <div className="divide-y divide-white/5">
          {recent.length === 0 ? (
            <div className="p-8 text-center text-gray-500">No collection yet.</div>
          ) : (
            recent.map(p => (
              <div key={p.id} className="p-4 flex justify-between">
                <div>
                  <p className="text-white">{p.user?.name}</p>
                  <p className="text-gray-400 text-sm">{p.method}</p>
                </div>
                <p className="text-neon-green font-bold">৳{p.amount}</p>
              </div>
            ))
          )}
        </div>
