...
export default async function ResellerDashboard() {
  ...
  // Fetch monthly income data from DB (last 6 months) for reseller wallet only
  ...
  // Query daily usage from data_usage (last 7 days) for reseller's customers
  const dailyUsageResult = await db
    ...
    .from(dataUsage)
    .where(and(sql`${dataUsage.recordedAt} >= current_date - interval '7 days'`, sql`${dataUsage.userId} in (select id from users where reseller_id = ${resellerId})`))
