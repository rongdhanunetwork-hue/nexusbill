...
async function countExpiredDays(resellerId: number, days: number) {
  const [result] = await db
    .select({ count: sql<number>`cast(count(*) as int)` })
    .from(users)
    .where(sql`${users.role} = 'customer' and ${users.resellerId} = ${resellerId} and ${users.expireDate}::date = (current_date - ${days}::int)`);
...
export async function GET() {
  ...
  try {
    const [
      ...
      countExpiredDays(resellerId, 1),
      countExpiredDays(resellerId, 2),
      countExpiredDays(resellerId, 3),
      countExpiredDays(resellerId, 4),
      ...
      db.select({ userId: payments.userId })
        .from(payments)
        .where(sql`${payments.status} = 'approved' and ${payments.createdAt} >= ${startOfMonth} and ${payments.userId} in (select id from users where reseller_id = ${resellerId})`),
    ]);
    ...
    const dailyUsageResult = await db
      ...
      .where(sql`${dataUsage.recordedAt} >= current_date - interval '7 days' and ${users.resellerId} = ${resellerId}`)
    ...
    console.error("Reseller dashboard stats error:", err);
